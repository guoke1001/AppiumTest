...
def run_pytest(device_info):
    pytest.main([f"--cmdopt={device_info}","--alluredir","./report/allure_result"])
...
def generate_report():
    ...
    files = os.listdir(global_config.REPORT_HISTORY_PATH)
    result_history_dir = os.path.join(global_config.REPORT_RESULT_PATH, "history")
        # 如果不存在则先创建文件夹
    if not os.path.exists(result_history_dir):
        print(os.mkdir(result_history_dir))
    for file in files:
        shutil.copy(os.path.join(global_config.REPORT_HISTORY_PATH, file), result_history_dir)
